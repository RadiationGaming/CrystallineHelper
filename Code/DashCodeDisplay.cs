...
namespace vitmod {
    ...
    [Tracked]
    public class DashCodeDisplay : Entity {
        ...
        public override void Update() {
            ...
            bool show = currentCodeArrows != null && VitModule.Settings.DisplayDashSequence;
        ...
        public void InitializeDashCodes(Level level) {
            ...
            dashCodes.Clear();
            doNotRemove = true;
            DashCodeController[] controllers = level.Tracker.GetEntities<DashCodeController>().Cast<DashCodeController>().ToArray();
            if (controllers.Length != 0) {
                foreach (DashCodeController controller in controllers)
        ...
        public void Fail() {
            if (codePosition > 0)
                Audio.Play("event:/vitellary/dash_sequence_wrong");
            codePosition = 0;
        }
        ...
        private static void Level_LoadLevel(On.Celeste.Level.orig_LoadLevel orig, Level self, Player.IntroTypes playerIntro, bool isFromLoader) {
            ...
            DashCodeDisplay display = self.Tracker.GetEntity<DashCodeDisplay>();
            if (display == null)
                self.Add(display = new DashCodeDisplay());
